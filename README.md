...
點選導覽列「匯出日曆圖」，可預覽並下載 1920 × 1080（16:9）PNG。匯出視窗可獨立切換週一至週五／週一至週日；使用開啟時的週次、組別、人數門檻與臨時異動快照。右側統計以一小時格數計算，圖片會註明開啟時的同步狀態。手機下載後也可長按圖片儲存。

執行 `node tests/calendar-export.mjs` 可用本機 Chrome 與隔離示範資料驗證預覽切換、PNG 尺寸及手機版面，不連線 Firebase。
